...
function TableIndex() {
  ...
  const calculateRedemption = () => {
    let total = 0;
    data.forEach((ele) => {
      ele["Campaign Code Redemption Status"] == "Yes" && total++;
    });
    setRedemptionStatus(total);
  };
    ...
    <div className="w-full relative px-3">
      <div className="sec m-auto my-10 space-y-8 w-1/2 mob:w-full flex flex-col">
        ...
        <div className="info flex mob:flex-col mob:justify-center mob:items-center mob:space-y-10 mob:p-5 justify-evenly space-x-3 mob:space-x-0">
          ...
          <div className="eligibleforswag w-fit mob:w-full h-20 p-5 space-x-5 rounded-lg flex flex-row justify-evenly mob:justify-between items-center bg-pink-50 shadow-lg shadow-blue-300/30 border border-blue-200">
            <p className="text-center mob:text-start text-sm text-blue-400">
              Total No of <br />
              Participants
            </p>
